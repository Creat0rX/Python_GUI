...
l_label = tk.Frame(dock, height=48, width=100, bg=l_color.get())
r_label = tk.Frame(dock, height=48, width=100, bg=r_color.get())
l_label.place(x=2, y=2)
r_label.place(x=2, y=50)

black = tk.Frame(dock, height=48, width=100, bg="black")
white = tk.Frame(dock, height=48, width=100, bg="white")
cyan = tk.Frame(dock, height=48, width=100, bg="cyan")
magenta = tk.Frame(dock, height=48, width=100, bg="magenta")

black.place(x=598, y=2)
white.place(x=598, y=50)
cyan.place(x=698, y=2)
magenta.place(x=698, y=50)
...
filemenu.add_command(label="New", command=lambda: canvas.delete("all"), underline=0, accelerator="Ctrl+N")
bgmenu = tk.Menu(filemenu, tearoff=0)
filemenu.add_cascade(label="Background colour", menu=bgmenu, underline=0)
bgmenu.add_command(label="White", command=lambda: canvas.config(bg="white"), underline=0, accelerator="Ctrl+W")
bgmenu.add_command(label="Black", command=lambda: canvas.config(bg="black"), underline=0, accelerator="Ctrl+B")
bgmenu.add_command(label="Cyan", command=lambda: canvas.config(bg="cyan"), underline=0, accelerator="Ctrl+C")
bgmenu.add_command(label="Magenta", command=lambda: canvas.config(bg="magenta"), underline=0, accelerator="Ctrl+M")
filemenu.add_separator()
filemenu.add_command(label="Quit", command=app.destroy, underline=0, accelerator="Ctrl+Q")

app.bind("<Control-n>", lambda dummy: canvas.delete("all"))
app.bind("<Control-q>", lambda dummy: app.destroy())
app.bind("<Control-w>", lambda dummy: canvas.config(bg="white"))
app.bind("<Control-b>", lambda dummy: canvas.config(bg="black"))
app.bind("<Control-c>", lambda dummy: canvas.config(bg="cyan"))
app.bind("<Control-m>", lambda dummy: canvas.config(bg="magenta"))
app.mainloop()